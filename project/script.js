...
function handleStampAcquisition(id, bypassLocationCheck = false) {
    if (!stamps[id]) return;
...
document.addEventListener('DOMContentLoaded', function () {
    function checkUrlParams() {
        ...
        if (id === '0808') {
            ...
            const ids = id.split(',');
            ids.forEach(stampId => {
                handleStampAcquisition(stampId, true);
            });
        }
    }
    ...
    if (usePointsButton) {
        usePointsButton.addEventListener('click', usePointsWithPassword);
    }
